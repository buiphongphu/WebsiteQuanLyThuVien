...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $_SESSION['error_message'] = "ID loại sách không hợp lệ";
    header("Location: ?url=admin/quanlyloaisach");
    exit();
}

$loaiSach = $loaiSachController->detail($_GET['id']);

if (!$loaiSach) {
    $_SESSION['error_message'] = "Không tìm thấy loại sách";
    header("Location: ?url=admin/quanlyloaisach");
    exit();
}
?>
    ...
    <div class="container mx-auto flex flex-wrap py-6">
        ...
        <main class="w-full md:w-3/4 px-2">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-2xl font-bold mb-4">Sửa Loại Sách</h2>
                ...
                <form action="?url=admin/quanlyloaisach/update" method="post">
                    <input type="hidden" name="Id" value="<?php echo $loaiSach['Id']; ?>">
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            <label for="TenLoai" class="block text-gray-700 text-sm font-bold mb-2">Tên Loại:</label>
                            <input type="text" name="TenLoai" id="TenLoai" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $loaiSach['TenLoai']; ?>">
                        </div>
                    </div>

    ...
    <main class="w-full md:w-3/4 px-2">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex justify-between items-center mb-4">
                <a href="?url=admin/quanlydocgia/add" class="bg-blue-500 text-white p-2 rounded-md mb-4 inline-block">
                    <i class="fas fa-plus"></i> Thêm Độc Giả
                </a>
                <form action="?url=admin/quanlydocgia/import" method="post" enctype="multipart/form-data" class="flex items-center">
                    <label for="fileInput" class="bg-blue-500 text-white p-2 rounded-md mb-4 inline-block cursor-pointer">
                        <i class="fas fa-file-import"></i> Nhập File
                    </label>
                    <input type="file" id="fileInput" name="file" class="hidden" onchange="this.form.submit()">
                </form>
                <div class="flex items-center">
                    <input type="text" id="searchInput" name="keyword" class="border border-gray-300 p-2 rounded-md" placeholder="Tìm kiếm độc giả...">
            ...
            <div id="searchResults">
                    ...
                    <div class="bg-white shadow-md rounded-lg overflow-x-auto">
                        <table class="min-w-full bg-white border border-gray-300 rounded-lg">
                            ...
                            <tr class="bg-gray-50">
                                ...
                                <th class="py-2 px-3 border-b border-gray-300 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày Sinh</th>
                                <th class="py-2 px-3 border-b border-gray-300 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Giới Tính</th>
                                ...
                                <th class="py-2 px-3 border-b border-gray-300 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Đơn Vị</th>
                                <th class="py-2 px-3 border-b border-gray-300 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mã Số</th>
                            ...
                            <?php foreach ($docGias as $docGia): ?>
                                <tr class="hover:bg-gray-100">
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['Id']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['Ho'] . ' ' . $docGia['Ten']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['NgaySinh']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['GioiTinh']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['DiaChi']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['DonVi']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['MaSo']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['SDT']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 text-sm text-gray-900"><?php echo $docGia['Email']; ?></td>
                                    <td class="py-2 px-3 border-b border-gray-300 flex space-x-2">
                                        <button class="bg-red-500 text-white p-2 rounded-md flex items-center" onclick="confirmDelete(<?php echo $docGia['Id']; ?>)">
                ...
                <?php else: ?>
                    <p>Không có độc giả nào được tìm thấy.</p>
                <?php endif; ?>
            </div>
        </div>
    </main>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Xóa Độc Giả</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Bạn có chắc chắn muốn xóa độc giả này? Thao tác này không thể hoàn tác.</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function confirmDelete(id) {
        ...
        document.getElementById('confirmDeleteBtn').onclick = function () {
            window.location.href = '?url=admin/quanlydocgia/delete&id=' + id;
        };
    }

    function searchDocGia() {
        const keyword = document.getElementById('searchInput').value;
        fetch('?url=admin/quanlydocgia/search', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                ...
                <tr>
                    ...
                    <th class="py-2 px-3 border-b border-gray-300 text-left">Ngày Sinh</th>
                    <th class="py-2 px-3 border-b border-gray-300 text-left">Giới Tính</th>
                    <th class="py-2 px-3 border-b border-gray-300 text-left">Địa Chỉ</th>
                    <th class="py-2 px-3 border-b border-gray-300 text-left">Đơn Vị</th>
                    <th class="py-2 px-3 border-b border-gray-300 text-left">Mã Số</th>
            ...
            `;
                    ...
                    data.forEach(docgia => {
                        ...
                        tr.innerHTML = `
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.Id}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.Ho} ${docgia.Ten}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.NgaySinh}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.GioiTinh}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.DiaChi}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.DonVi}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.MaSo}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.SDT}</td>
                    <td class="py-2 px-3 border-b border-gray-300">${docgia.Email}</td>
                    <td class="py-2 px-3 border-b border-gray-300 flex space-x-2">
                        <button class="bg-red-500 text-white p-2 rounded-md flex items-center" onclick="confirmDelete(${docgia.Id})">
                            <i class="fas fa-trash-alt mr-1"></i> Xóa
                        </button>
                    </td>
                `;
                    ...
                    // Add scrollbar to searchResults if needed
                    searchResults.style.overflowY = 'auto';
                    searchResults.style.maxHeight = '400px'; // Adjust max-height as needed
                } else {
                    searchResults.innerHTML = '<p>Không có độc giả nào được tìm thấy.</p>';
                }
            })
